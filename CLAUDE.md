...
**งานถัดไปที่ยังไม่ทำ:**
- **🟡 content-layer: raw body-prose links ยังเสีย ~1,585 จุด (342 review + 12 article × 7 ภาษา):** link() ของ 3 layout **แก้แล้ว** (2026-07-18, commit 0c8e89b6 — nav/footer/breadcrumb/related route ถูกด้วย `localizedSlugSet()` ใน `lib/locales.ts`: /<loc>/ ถ้ามี twin ไม่งั้น /en/). **ที่เหลือ = `<a href="slug.html">` ดิบใน descHtml/intro/block prose ของ content JSON ที่ origin แปล** — render ผ่าน `set:html` เลยไม่ผ่าน link() → บนหน้า /ar/ resolve เป็น /ar/slug 404. **fix:** ต้องเขียน helper regex rewrite href ใน body HTML string ก่อน set:html (ใช้ localizedSlugSet เดิม route /<loc>/ vs /en/) แล้ว apply ทุกจุด set:html body ใน 3 layout (~40 จุด) — invasive + ต้อง RTL-safe, ยังไม่ทำ. hub-layer (`localize.mjs rewriteUrls`, 45k ลิงก์) + content-layer link() เป็นแบบอ้างอิงได้
