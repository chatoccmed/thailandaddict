...
- **🟢 ลิงก์เสียทั้งเว็บ: 84,907 → 126 (99.85% ลด, deploy แล้ว)** ผ่าน 4 commit (2026-07-18): hub-layer `localize.mjs rewriteUrls` (45k), content-layer `link()` 3-way ทั้ง 3 layout + `localizedSlugSet()`/`localizedSlugSet('en')` ใน `lib/locales.ts` (in-locale twin→/<loc>/ · EN page→/en/ · root-only→/), prose rewrite ใน content-<loc> JSON (1,015), structured card href wrap (it.href/c.href/cta). **audit-site.mjs รู้จัก /go/b /api/* worker route แล้ว.**
  - **126 ที่เหลือ (ยังไม่แก้ — diminishing returns):** (1) **EN-page routing** `/en/trip` root-only ×49 + `/en/en/*` double-prefix ×56 — pre-existing, การแก้ต้อง re-route ทั้ง 6,800 EN page = เสี่ยง; (2) **structured href บาง block ที่ยังไม่ wrap link()** — `staycta-btn`, guides/essential-links (เช่น `href="top10-hotels-bangkok.html"`, `thailand-travel-budget.html` บนหน้า /<loc>/) render ตรงด้วย `target="_blank"` เลย bypass link() → หาให้ครบทุก `href={x.href}` ใน ArticleLayout แล้ว wrap ด้วย link() (ระวัง target="_blank" internal); (3) content gap จริง: `thailand-travel-budget`, `top10-hotels-pattaya-beachfront/jomtien`, `review-dusit-thani-beach-resort-krabi` (หน้าไม่มี/rename)
