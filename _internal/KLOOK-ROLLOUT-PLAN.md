...
- **2026-07-18 12:55 — deploy #3 ✅** version `1524f696`. Shipped Krabi 10/10, Pattaya 10/10, Samui 3/10, Chiang Mai 3/10. Verified live: the Sanctuary of Truth, Krabi 4-island, Samui Koh Tao and CM Night Safari deep links all render on thailandaddict.com; homepage 200, /go/b 302. Winning move after #2 failed: wait for the other session's build to finish, then deploy ITS dist — it already contains our committed content, so there is no need to build our own.
- **2026-07-18 14:31 — deploy #6 ✅** version `3b6eeaa7` (4,741 assets). Shipped Ayutthaya 6/10, Kanchanaburi 4/10, Chiang Rai 3/10 — all verified live. A background "wait for a window, then deploy, retry on failure" job landed it: the first attempt still collided, it retried itself, the second cleared.
- **⚠️ Verify deploys with a cache-buster.** The site sends `Cache-Control: max-age=3600`, so a plain curl right after a deploy returns `CF-Cache-Status: HIT` and the OLD page — which looks exactly like a failed deploy. Append `?cb=<timestamp>` and re-check before diagnosing anything. This cost real confusion once already.
- **Deploys #4/#5 collided again (14:23)** — the other session rebuilds every few minutes, so a manual "is dist quiet?" check right before deploying is not enough: the risky window is the whole upload, and a big rebuild means thousands of new asset hashes (7,202 files) and a long upload. Two things help: (a) a failed attempt still uploads what it managed, so each retry has fewer files left and a shorter window; (b) `wrangler deploy --assets <dir>` can point at a snapshot copy of dist, which a concurrent build cannot pull files out from under — worth the ~4GB copy when the disk has room. Best of all, just wait for a window in the background and fire then. **Nothing is ever lost by losing the race:** committed content ships with whichever session deploys next.
- **⚠️ Disk during a build:** free space dropped to **4.1GB (commit 6.0GB)** at the build's peak, under the >9GB a build needs. Deleting `~/ta-build-temp` (build-test's reusable temp, ~1.2GB, safe when no `.lock` is held) took it back to 11GB / 15.1GB commit. Check this before blaming build failures on code.
- **Deploy #2 failed exactly this way (12:51)** — build was fine (17,109 pages), but the other session started its own build *during* wrangler's asset upload, wiping dist. Wrangler died at `ENOENT … dist\zh\404.html` after 517/776 assets. **Production was unaffected** — a failed upload never activates, so the site kept serving the previous good version (verified: /, an activity page, /en/, and the /go/b worker all still healthy). Lesson: "dist quiet" before starting is not enough, since the window spans the whole upload. Better: wait for dist to be BOTH complete (>15k html) and quiet, deploy immediately, and if it fails just retry — content that is committed ships with whichever session deploys next, so there is nothing to race over.
