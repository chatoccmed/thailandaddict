...
function inject(file, loc) {
  ...
  /* Since 2026-09-12 the homepage is generated whole by
     _internal/shell/build/gen-proto-home.mjs, which reads the same content this
     file reads and needs no markers. Every replace() below is a no-op on a page
     that has none — String.replace does not throw on a miss — so without this
     guard the generator would rewrite the file byte-for-byte and report
     "PROV 89 provinces" as if it had done something. Say so instead. */
  if (!/\/\*GEN:PROV\*\//.test(h)) {
    console.log(`[${loc}] ${path.relative(ROOT, file)} · no GEN markers — generated by gen-proto-home.mjs, nothing to inject`);
    return;
  }
