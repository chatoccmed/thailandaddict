export const meta = {
  name: 'fetch-prachinburi-imgs',
  description: 'Fetch real photos for 1 Prachinburi hotel (Baan Mai Chai Klong Resort) — missing heroImg+gallery base-1..4. Tripadvisor/Ostrovok/official, NOT Trip.com/Agoda. Optimize. Never fake.',
  phases: [{ title: 'Fetch', detail: '1 agent: find real photos, curl base-1..4, optimize' }],
}

const HOTELS = [
  { name: 'บ้านไม้ชายคลองรีสอร์ท (Baan Mai Chai Klong Resort), ปราจีนบุรี', base: 'prachinburi-baanmaichaiklong' },
];
...
const res = await parallel(HOTELS.map(h => () => {
  ...
  return agent(`ดึงรูปจริงของ "${h.name}" — รูปถ่ายโรงแรม/รีสอร์ทจริงเท่านั้น
แหล่ง: Tripadvisor (dynamic-media-cdn.tripadvisor.com) / Ostrovok (cdn.worldota.net) / เว็บ/เพจทางการของรีสอร์ท — **ห้าม Trip.com, Agoda, รูป stock/ไม่เกี่ยว**
web-search หา gallery → URL จริง 4 รูป (หน้า/ห้อง/ริมคลอง/บรรยากาศ) → curl -m 60 -A "Mozilla/5.0" (รูปแรก=hero):
