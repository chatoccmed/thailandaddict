...
// === Nan (cluster=nan): bo-kluea/nature/city ===
const PROV = 'น่าน', CLUSTER = 'nan', CRUMB = 'น่าน', CRUMBHREF = 'city-nan.html';
const POSTS = [
  { id:3819, old:'top10-boklua-nan-hotels', newSlug:'top12-bo-kluea-sapan-nature-hotels-nan', title:'12 ที่พักสัมผัสธรรมชาติ บ่อเกลือ-สะปัน น่าน', n:12 },
  { id:528, old:'top10-nan-nature-hotels', newSlug:'top10-nature-hotels-nan', title:'10 ที่พักอิงธรรมชาติ สูดโอโซน จังหวัดน่าน', n:10 },
  { id:529, old:'top10-nan-hotel', newSlug:'top10-nan-city-hotels', title:'10 โรงแรมในอำเภอเมืองน่าน', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-nan'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:nan', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/nan-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
