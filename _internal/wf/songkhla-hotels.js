export const meta = {
  name: 'songkhla-hotels',
  description: 'Songkhla (สงขลา/หาดใหญ่) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Songkhla old town/Hat Yai/Samila hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมสงขลา roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-songkhla.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-songkhla" (EN /en/) · crumbCityName="สงขลา"/(EN "Songkhla"), crumbCityHref="city-songkhla.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/songkhla-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-songkhla'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดสงขลา ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: โรงแรม 4-5 ดาวหาดใหญ่ 3-4 · บูทีค/โรงแรมเมืองเก่าสงขลา-หาดสมิหลา 3 · กลาง 3 · ประหยัด/โฮสเทล 2 — กระจายย่าน (เมืองเก่าสงขลา/หาดสมิหลา · หาดใหญ่ · เกาะยอ/ริมทะเลสาบ)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ (หาดใหญ่มีโรงแรมเยอะ — เลือกที่เปิดจริงและมีรีวิว)
slug = review-<ชื่อโรงแรม-kebab>-songkhla · คืนผลตาม schema`,
  { label:'plan:songkhla-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" สงขลา · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมสงขลา" (top10-hotels-songkhla)
...
สร้าง roundup "Top 10 โรงแรมสงขลา": เขียน astro/src/content/roundups/top10-hotels-songkhla.json (ไทย) + roundups-en/top10-hotels-songkhla.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-songkhla" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-songkhla)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:songkhla', phase:'Roundup' }
)
