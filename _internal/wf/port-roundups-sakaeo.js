...
// === Sa Kaeo (cluster=sa-kaeo): aranyaprathet + city ===
const PROV = 'สระแก้ว', CLUSTER = 'sa-kaeo', CRUMB = 'สระแก้ว', CRUMBHREF = 'city-sa-kaeo.html';
const POSTS = [
  { id:497, old:'top10-aranyaprathet-hotels', newSlug:'top10-aranyaprathet-hotels-sa-kaeo', title:'10 ที่พักในอรัญประเทศ สระแก้ว', n:10 },
  { id:498, old:'top9-sakaeo-hotels', newSlug:'top9-sa-kaeo-city-hotels', title:'9 ที่พักในอำเภอเมืองสระแก้ว', n:9 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-sa-kaeo'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:sakaeo', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/sa-kaeo-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
