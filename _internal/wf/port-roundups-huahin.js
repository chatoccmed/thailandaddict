...
// === Hua Hin chunk (cluster=huahin): 6 overlapping lists ===
const PROV = 'ประจวบคีรีขันธ์', CLUSTER = 'huahin', CRUMB = 'หัวหิน', CRUMBHREF = 'city-huahin.html';
const POSTS = [
  { id:3958, old:'top10-5star-hotels-hua-hin', newSlug:'top10-luxury-5star-hotels-huahin', title:'10 โรงแรมหรู 5 ดาว ริมหาดหัวหิน-ชะอำ', n:10 },
  { id:1909, old:'top-15-huahin-hotels', newSlug:'top15-popular-hotels-huahin', title:'15 ที่พักยอดนิยม ทำเลดี ใกล้หาดหัวหิน', n:15 },
  { id:763, old:'top10-khao-takiab-beach-huahin-hotel', newSlug:'top10-khao-takiab-beach-hotels-huahin', title:'10 ที่พักริมหาดเขาตะเกียบ หัวหิน', n:10 },
  { id:979, old:'top10-night-market-huahin-hotels', newSlug:'top10-night-market-hotels-huahin', title:'10 ที่พักใกล้ตลาดกลางคืน ใจกลางหัวหิน', n:10 },
  { id:3960, old:'top10-prachuap-khiri-khan-hotels', newSlug:'top8-budget-hotels-huahin', title:'8 ที่พักหลักร้อยในหัวหิน', n:8 },
  { id:3953, old:'5-hostel-huahin', newSlug:'top5-hostels-huahin', title:'5 Hostel ยอดนิยมในหัวหิน', n:5 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-huahin'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:huahin', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/huahin-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
