export const meta = {
  name: 'khaosok-reviews-en',
  description: 'Translate 6 Khao Sok reviews into English twins',
...
const EN_TEMPLATE = EN_DIR + '\\review-khao-sok-rafthouse-surat-thani.json'
...
const results = await parallel(SLUGS.map(slug => () =>
  agent(
    `Translate a finalized Thai review into its ENGLISH twin for thailandaddict.com (/en/).

STEP 1 — Read:
  • SOURCE (finalized Thai review): ${TH_DIR}\\${slug}.json
  • EN CONVENTIONS REFERENCE (an existing Khao Sok English review twin — copy its structure/href style): ${EN_TEMPLATE}
...
2. Translate ALL Thai text to natural, fluent English (friendly travel-writer register): title, metaDesc, keywords, ogTitle, ogDesc, twDesc, schemaDesc, h1, intro, heroSub1/2, badge*, hi*, qi* labels+values, body[].html (keep the >=2000-char depth), highlights, ratingBars labels, booking/agoda pros+cons, honestSummary, honestChecks, rooms[].name+full, tips, info, galleryAlts, faq, quickAnswerHtml, nav/parent/crumb labels, heroCredit. Romanize Thai place names (สุราษฎร์ธานี->Surat Thani, เขาสก->Khao Sok, ทะเลสาบเชี่ยวหลาน->Cheow Lan Lake, เขื่อนรัชชประภา->Ratchaprapha Dam, คลองสก->Khlong Sok, แม่น้ำสก->Sok River, อุทยานแห่งชาติ->national park).
