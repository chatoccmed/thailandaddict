export const meta = {
  name: 'koh-kood-hotels',
  description: 'Koh Kood (เกาะกูด, ตราด) destination — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Koh Kood hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 ที่พักเกาะกูด roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-koh-kood.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-koh-kood" (EN /en/) · crumbCityName="เกาะกูด"/(EN "Koh Kood"), crumbCityHref="city-koh-kood.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: หารูปจริงของโรงแรม (Booking/Agoda/Trip/Google) โหลดไป astro/public/images/hotels/koh-kood-<short>.jpg (curl -m 60 --connect-timeout 20) ครบ hero+gallery ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี gradient placeholder)
- ราคา "เริ่มประมาณ" จากช่วงราคาห้องมาตรฐานจริง ไม่เอาราคาพีคจุดเดียว · เกาะกูดราคาห้องสวิงตามฤดู (โลว์ซีซั่นหลายที่ปิด) ใช้ราคาช่วงปกติ
...
if (existingReviews.length >= 10) {
  ...
  const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:12, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
    name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-koh-kood (ต้องไม่ซ้ำกับตัวอื่น และต้องลงท้าย -koh-kood)'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

  const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/รีสอร์ตบนเกาะกูด (จ.ตราด) ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: รีสอร์ตหรู/บีชฟรอนต์ 3-4 · บูทีค 3 · กลาง 3 · ประหยัด/บังกะโล 1-2 · โฮสเทล 1 — กระจายย่าน (อ่าวตะเภา · อ่าวพร้าว · อ่าวง่ามโข่/คลองเจ้า · บางเบ้า · อ่าวสลัด)
**ห้ามเลือกซ้ำชื่อโรงแรมเดียวกัน** เลือกที่มีรีวิวจริงและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
**ต้องได้ 12 ที่ slug ไม่ซ้ำกันเลย และเป็นคนละโรงแรมกันจริง ๆ** — slug = review-<ชื่อ-kebab>-koh-kood (ลงท้าย -koh-kood เสมอ ห้ามลงท้าย -trat เพราะชนคลัสเตอร์ตราด) · คืนผลตาม schema (12 รายการพอดี)`,
    { label:'plan:koh-kood-hotels', phase:'Plan', schema: PLAN_SCHEMA }
  )
  ...
  const reviewed = await parallel(hotels.map(h => () =>
    agent(
`รีวิวโรงแรม "${h.name}" เกาะกูด ตราด · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 ที่พักเกาะกูด" (top10-hotels-koh-kood)
...
สร้าง roundup "Top 10 ที่พักเกาะกูด": เขียน astro/src/content/roundups/top10-hotels-koh-kood.json (ไทย) + roundups-en/top10-hotels-koh-kood.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-koh-kood" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-koh-kood)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:koh-kood', phase:'Roundup' }
)

return { hotels: hotels.length, reviewsOk: okR.length, roundup: 'top10-hotels-koh-kood' }
