export const meta = {
  name: 'surat-thani-hotels',
  description: 'Surat Thani gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Surat Thani hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมสุราษฎร์ธานี roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-surat-thani.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-surat-thani" (EN ใช้ /en/) · crumbCityName="สุราษฎร์ธานี"/(EN "Surat Thani"), crumbCityHref="city-surat-thani.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมสุราษฎร์ธานีตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/surat-thani-<short>.jpg (ใช้ curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-surat-thani'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดสุราษฎร์ธานีที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
สุราษฎร์ธานีครอบคลุมทั้งเกาะสมุย เกาะพะงัน เกาะเต่า เขาสก-เชี่ยวหลาน และตัวเมือง — คละให้ครบ: เกาะสมุย (เฉวง/ละไม/บ่อผุด) 4 · เกาะพะงัน 1 · เกาะเต่า 1 · แพ/รีสอร์ทเขาสก-เชี่ยวหลาน 2 · ตัวเมืองสุราษฎร์ 2-3
คละระดับ: หรู 2-3 · บูทีค/4-5 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 1-2
เลือกที่ที่มีรีวิวจริงเยอะและจองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-surat-thani
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:surat-thani-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" สุราษฎร์ธานี · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมสุราษฎร์ธานี" (top10-hotels-surat-thani)
...
สร้างหน้า roundup "Top 10 โรงแรมสุราษฎร์ธานี" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-surat-thani.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-surat-thani.json (อังกฤษ)
...
slug="top10-hotels-surat-thani" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-surat-thani)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:surat-thani', phase:'Roundup' }
)
