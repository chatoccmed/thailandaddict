export const meta = {
  name: 'kanchanaburi-hotels',
  description: 'Kanchanaburi gold-template — 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    ...
    { title: 'Roundup', detail: 'build Top 10 โรงแรมกาญจนบุรี roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-kanchanaburi.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-kanchanaburi" (EN /en/) · crumbCityName="กาญจนบุรี"/(EN "Kanchanaburi"), crumbCityHref="city-kanchanaburi.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/kanchanaburi-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-kanchanaburi'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดกาญจนบุรีที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: หรู 2 · บูทีค/4-5 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 2 — กระจายย่าน (แพ/รีสอร์ทริมแม่น้ำแคว/ตัวเมือง/ไทรโยค/ศรีสวัสดิ์-เขื่อนศรีนครินทร์/สังขละบุรี) เน้นที่พักแพริมน้ำที่เป็นเอกลักษณ์
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
slug = review-<ชื่อโรงแรม-kebab>-kanchanaburi · คืนผลตาม schema`,
  { label:'plan:kanchanaburi-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" กาญจนบุรี · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมกาญจนบุรี" (top10-hotels-kanchanaburi)
...
สร้าง roundup "Top 10 โรงแรมกาญจนบุรี": เขียน astro/src/content/roundups/top10-hotels-kanchanaburi.json (ไทย) + roundups-en/top10-hotels-kanchanaburi.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-kanchanaburi" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-kanchanaburi)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:kanchanaburi', phase:'Roundup' }
)
