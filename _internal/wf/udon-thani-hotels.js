export const meta = {
  name: 'udon-thani-hotels',
  description: 'Udon Thani (อุดรธานี) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Udon Thani hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมอุดรธานี roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-udon-thani.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-udon-thani" (EN /en/) · crumbCityName="อุดรธานี"/(EN "Udon Thani"), crumbCityHref="city-udon-thani.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/udon-thani-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-udon-thani'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดอุดรธานี ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: โรงแรม 5 ดาว/หรูในเมือง 2-3 · บูทีค/4 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 1 — กระจายย่าน (ใจกลางเมืองอุดร · ริมหนองประจักษ์ · ย่าน UD Town/เซ็นทรัล · ใกล้สนามบินอุดร)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ (อุดรเป็นเมืองใหญ่มีโรงแรมธุรกิจและโรงแรมเครือเยอะ — เลือกที่เปิดทั้งปี)
slug = review-<ชื่อโรงแรม-kebab>-udon-thani · คืนผลตาม schema`,
  { label:'plan:udon-thani-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" อุดรธานี · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมอุดรธานี" (top10-hotels-udon-thani)
...
สร้าง roundup "Top 10 โรงแรมอุดรธานี": เขียน astro/src/content/roundups/top10-hotels-udon-thani.json (ไทย) + roundups-en/top10-hotels-udon-thani.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-udon-thani" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-udon-thani)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:udon-thani', phase:'Roundup' }
)
