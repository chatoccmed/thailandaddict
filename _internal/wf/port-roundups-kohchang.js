...
// === Koh Chang (cluster=koh-chang): 4 overlapping lists ===
const PROV = 'ตราด', CLUSTER = 'koh-chang', CRUMB = 'เกาะช้าง', CRUMBHREF = 'city-koh-chang.html';
const POSTS = [
  { id:3070, old:'top15-hotels-koh-chang-trat', newSlug:'top15-hotels-koh-chang', title:'15 โรงแรมยอดนิยมบนเกาะช้าง ตราด', n:15 },
  { id:3074, old:'top-9-hotel-koh-chang-trat', newSlug:'top9-white-sand-beach-hotels-koh-chang', title:'9 ที่พักหาดทรายขาว เกาะช้าง ตราด', n:9 },
  { id:3076, old:'top-nine-koh-chang-hotel', newSlug:'top9-budget-hotels-koh-chang', title:'9 ที่พักหลักร้อยบนเกาะช้าง ตราด', n:9 },
  { id:3072, old:'top5-hostel-koh-chang', newSlug:'top4-hostels-koh-chang', title:'4 ที่พักสไตล์ Hostel สุดชิลบนเกาะช้าง ตราด', n:4 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-koh-chang'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:koh-chang', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/koh-chang-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
