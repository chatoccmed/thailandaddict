...
// === Bangkok (cluster=bangkok): 6 BTS-area lists (single-planner dedup adjacent stations) ===
const PROV = 'กรุงเทพฯ', CLUSTER = 'bangkok', CRUMB = 'กรุงเทพ', CRUMBHREF = 'city-bangkok.html';
const POSTS = [
  { id:2172, old:'topfive-hotels-bts', newSlug:'top5-sanam-pao-bts-hotels-bangkok', title:'5 ที่พักใกล้ BTS สนามเป้า กรุงเทพฯ', n:5 },
  { id:2189, old:'topnine-hotels-bts-mrt-morchit-chatuchak', newSlug:'top9-mochit-chatuchak-hotels-bangkok', title:'9 โรงแรมใกล้ BTS หมอชิต MRT จตุจักร กรุงเทพฯ', n:9 },
  { id:2178, old:'topfive-hotels-bangchak-bts', newSlug:'top5-bang-chak-bts-hotels-bangkok', title:'5 โรงแรมใกล้ BTS บางจาก กรุงเทพฯ', n:5 },
  { id:1530, old:'top11-ari-bts-hotels', newSlug:'top11-ari-bts-hotels-bangkok', title:'11 ที่พักใกล้ BTS อารีย์ กรุงเทพฯ', n:11 },
  { id:1341, old:'top10-bts-ratchathewi-bangkok-hotels', newSlug:'top10-ratchathewi-bts-hotels-bangkok', title:'10 โรงแรมใกล้ BTS ราชเทวี กรุงเทพฯ', n:10 },
  { id:965, old:'top-10-phayathai-bts-airport-link-hotels', newSlug:'top10-phaya-thai-bts-hotels-bangkok', title:'10 ที่พักใกล้ BTS พญาไท / Airport Link กรุงเทพฯ', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-bangkok'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:bkk', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/bangkok-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
