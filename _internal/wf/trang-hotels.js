export const meta = {
  name: 'trang-hotels',
  description: 'Trang (ตรัง) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Trang town/Pak Meng/island hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมตรัง roundup TH+EN' },
  ],
}
...
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/trang-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-trang'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดตรัง ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: รีสอร์ตริมทะเล/เกาะ (เกาะมุก/เกาะกระดาน/ปากเมง) 2-3 · บูทีค/4 ดาวในเมืองตรัง 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 1 — กระจายย่าน (ในเมืองตรัง · หาดปากเมง-ฉางหลาง · เกาะ (เกาะมุก/เกาะกระดาน) · กันตัง/สิเกา)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ (รีสอร์ตบนเกาะบางที่เปิดเฉพาะไฮซีซั่น — เลือกที่จองออนไลน์ได้จริงและมีรีวิว)
slug = review-<ชื่อโรงแรม-kebab>-trang · คืนผลตาม schema`,
  { label:'plan:trang-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" ตรัง · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
...
สร้าง roundup "Top 10 โรงแรมตรัง": เขียน astro/src/content/roundups/top10-hotels-trang.json (ไทย) + roundups-en/top10-hotels-trang.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-trang" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-trang)
