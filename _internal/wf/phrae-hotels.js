export const meta = {
  name: 'phrae-hotels',
  description: 'Phrae gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Phrae hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมแพร่ roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-phrae.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-phrae" (EN ใช้ /en/) · crumbCityName="แพร่"/(EN "Phrae"), crumbCityHref="city-phrae.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมแพร่ตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/phrae-<short>.jpg ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror) อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-phrae'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดแพร่ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
แพร่เป็นเมืองเหนือเล็ก ที่พักส่วนใหญ่เป็นบูทีค/โรงแรมในตัวเมืองและย่านเมืองเก่า โฮมสเตย์ในบ้านไม้สัก เกสต์เฮาส์ · บางส่วนเป็นรีสอร์ทชานเมือง/ใกล้พระธาตุช่อแฮ
คละระดับให้สมจริงกับเมืองเล็ก: บูทีค/โรงแรมชั้นดี 3 · กลาง 4 · ประหยัด/โฮมสเตย์/เกสต์เฮาส์ 3 · โฮสเทล 2 (ถ้ามีหรูจริงค่อยใส่) — กระจายทำเล (เมืองเก่า/กลางเมือง/ช่อแฮ/ชานเมือง)
เลือกที่ที่มีรีวิวจริงและจองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-phrae
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:phr-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" แพร่ · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมแพร่" (top10-hotels-phrae)
...
สร้างหน้า roundup "Top 10 โรงแรมแพร่" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-phrae.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-phrae.json (อังกฤษ)
...
slug="top10-hotels-phrae" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-phrae เพื่อดึงคะแนน/ราคา/ทำเล/ลิงก์จองมาใส่ entries)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:phr', phase:'Roundup' }
)
