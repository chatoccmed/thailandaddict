export const meta = {
  name: 'ratchaburi-hotels',
  description: 'Ratchaburi gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Ratchaburi / Suan Phueng hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมราชบุรี roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-ratchaburi.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-ratchaburi" (EN ใช้ /en/) · crumbCityName="ราชบุรี"/(EN "Ratchaburi"), crumbCityHref="city-ratchaburi.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมราชบุรีตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/ratchaburi-<short>.jpg ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror) อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-ratchaburi'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดราชบุรีที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
ราชบุรีมีที่พักหลายโซน: "สวนผึ้ง/บ้านคา" (รีสอร์ตวิวเขา-ริมธารเยอะที่สุด ตั้งแต่หรูถึงแคมป์) · "ตัวเมืองราชบุรี/ริมแม่น้ำแม่กลอง" (โรงแรม บูทีคในตึกเก่า/โรงโอ่ง) · "ดำเนินสะดวก" (ที่พักใกล้ตลาดน้ำ)
คละระดับให้สมจริง: รีสอร์ทหรู/วิวเขาสวนผึ้ง 2 · บูทีค/รีสอร์ท 4 · กลาง 3 · ประหยัด/โฮมสเตย์/แคมป์ 2 · โฮสเทล/เกสต์เฮาส์ 1 — กระจายทำเล (สวนผึ้ง/ตัวเมือง/ดำเนินสะดวก)
เลือกที่ที่มีรีวิวจริงเยอะและคนไทยรู้จัก/จองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-ratchaburi
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:rbi-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" ราชบุรี · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมราชบุรี" (top10-hotels-ratchaburi)
...
สร้างหน้า roundup "Top 10 โรงแรมราชบุรี" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-ratchaburi.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-ratchaburi.json (อังกฤษ)
...
slug="top10-hotels-ratchaburi" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-ratchaburi เพื่อดึงคะแนน/ราคา/ทำเล/ลิงก์จองมาใส่ entries)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:rbi', phase:'Roundup' }
)
