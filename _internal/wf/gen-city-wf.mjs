// Generate a self-contained per-city porter workflow from a config JSON.
// Usage: node _internal/wf/gen-city-wf.mjs <config.json>
// Config: { cluster, prov, crumb, crumbHref, posts:[{old,newSlug,title,n}] }
// Emits:  _internal/wf/port-<cluster>.js  (single-planner: plan -> write reviews >=2000w -> build roundups)
import fs from 'fs';

const cfgPath = process.argv[2];
if (!cfgPath) { console.error('need config path'); process.exit(1); }
const C = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
for (const k of ['cluster', 'prov', 'crumb', 'crumbHref', 'posts']) {
  if (!C[k]) { console.error('missing config field:', k); process.exit(1); }
}
...
const CLUSTER = ${J(C.cluster)}, PROV = ${J(C.prov)}, CRUMB = ${J(C.crumb)}, CRUMBHREF = ${J(C.crumbHref)};
const POSTS = ${J(C.posts)};
...
const out = `_internal/wf/port-${C.cluster}.js`;
fs.writeFileSync(out, script);
console.log('WROTE', out, '('+script.length+' bytes) for', C.posts.length, 'roundup(s)');
