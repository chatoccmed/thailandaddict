export const meta = {
  name: 'satun-hotels',
  description: 'Satun (สตูล) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Satun town/Koh Lipe/Pak Bara hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมสตูล roundup TH+EN' },
  ],
}
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-satun'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดสตูล ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: รีสอร์ตริมหาดเกาะหลีเป๊ะ 3-4 (luxury/boutique/midrange) · โรงแรมในเมืองสตูล 3 · ปากบารา/ละงู 2 · ประหยัด/โฮสเทล 2 — กระจายย่าน (เกาะหลีเป๊ะ หาดซันไรส์/พัทยา · ในเมืองสตูล · ปากบารา-ละงู)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ (รีสอร์ตหลีเป๊ะหลายที่ปิดหน้ามรสุม — เลือกที่จองออนไลน์ได้จริงและมีรีวิว)
slug = review-<ชื่อโรงแรม-kebab>-satun · คืนผลตาม schema`,
  { label:'plan:satun-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" สตูล · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
...
สร้าง roundup "Top 10 โรงแรมสตูล": เขียน astro/src/content/roundups/top10-hotels-satun.json (ไทย) + roundups-en/top10-hotels-satun.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-satun" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-satun)
