...
// === Chiang Rai (cluster=chiang-rai): nature + city ===
const PROV = 'เชียงราย', CLUSTER = 'chiang-rai', CRUMB = 'เชียงราย', CRUMBHREF = 'city-chiang-rai.html';
const POSTS = [
  { id:2638, old:'top-10-hotels-chang-rai', newSlug:'top10-nature-hotels-chiang-rai', title:'10 ที่พักอิงธรรมชาติในจังหวัดเชียงราย', n:10 },
  { id:2642, old:'top-eight-hotels-chiang-rai', newSlug:'top10-chiang-rai-city-hotels', title:'10 โรงแรมยอดนิยมในอำเภอเมืองเชียงราย', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-chiang-rai'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:cr', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/chiang-rai-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
