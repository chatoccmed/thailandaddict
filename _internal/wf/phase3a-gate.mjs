...
const P3 = [
  'top10-budget-hotels-prachuap-khiri-khan', 'top10-budget-hotels-nan', 'top10-budget-hotels-chiang-rai',
  'top10-budget-hotels-mae-hong-son', 'top10-budget-hotels-pai', 'top10-budget-hotels-trat',
  'top10-budget-hotels-sukhothai', 'top10-budget-hotels-ayutthaya', 'top10-budget-hotels-hat-yai',
];
...
console.log(`\n══ PHASE-3a GATE: ${totalFail ? `FAIL (${totalFail} roundups)` : 'PASS'} ══`);
const dataDir = path.join(ROOT, '_internal/phase1-data');
if (fs.existsSync(dataDir)) fs.writeFileSync(path.join(dataDir, 'phase3a-images.txt'), [...newImages].filter(i => fs.existsSync(path.join(PUB, i))).join('\n'));
console.log(`${newImages.size} distinct entry images referenced (list → _internal/phase1-data/phase3a-images.txt) — verify on R2 before deploy.`);
