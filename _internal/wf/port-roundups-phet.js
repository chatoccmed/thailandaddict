...
// === Phetchabun (cluster=phetchabun): khao-kho x2 + city ===
const PROV = 'เพชรบูรณ์', CLUSTER = 'phetchabun', CRUMB = 'เพชรบูรณ์', CRUMBHREF = 'city-phetchabun.html';
const POSTS = [
  { id:634, old:'top10-khao-kho-phetchabun', newSlug:'top10-khao-kho-romantic-hotels-phetchabun', title:'10 โรงแรมโรแมนติกทะเลหมอก บนเขาค้อ เพชรบูรณ์', n:10 },
  { id:633, old:'top10-cheap-khao-kho-phetchabun-hotels', newSlug:'top10-budget-khao-kho-hotels-phetchabun', title:'10 ที่พักราคาประหยัด บรรยากาศดี บนเขาค้อ เพชรบูรณ์', n:10 },
  { id:710, old:'top10-phetchabun-hotels', newSlug:'top10-phetchabun-city-hotels', title:'10 โรงแรมบรรยากาศดีใจกลางเมืองเพชรบูรณ์', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-phetchabun'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:phet', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/phetchabun-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
