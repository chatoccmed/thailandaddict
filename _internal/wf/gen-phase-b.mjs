...
for (const it of L.items) {
  if (it.kind !== 'review' || it.status === 'checked' || qa.has(it.cluster)) continue;
...
const script = `export const meta = {
  name: 'deep-audit-phase-b',
  description: 'Phase-B deep audit — ${nrev} reviews across ${Object.keys(byCluster).length} long-tail clusters, ${batches.length} batch(es). Web-verify each hotel real+open, score/price plausible, hero matches. No edits.',
...
`;
fs.writeFileSync('_internal/wf/deep-audit-phase-b.js', script);
console.log(`WROTE _internal/wf/deep-audit-phase-b.js — ${nrev} reviews, ${Object.keys(byCluster).length} clusters, ${batches.length} batches`);
