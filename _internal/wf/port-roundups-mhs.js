...
// === Mae Hong Son city (cluster=mae-hong-son): 1 roundup ===
const PROV = 'แม่ฮ่องสอน', CLUSTER = 'mae-hong-son', CRUMB = 'แม่ฮ่องสอน', CRUMBHREF = 'city-mae-hong-son.html';
const POSTS = [
  { id:2631, old:'top-ten-hotels-mae-hong-son', newSlug:'top10-mae-hong-son-city-hotels', title:'10 ที่พักบรรยากาศดีในเมืองแม่ฮ่องสอน', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-mae-hong-son'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:mhs', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/mae-hong-son-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
