export const meta = {
  name: 'port-phang-nga',
  description: 'Single-planner porter for พังงา (cluster=phang-nga): one agent plans all 1 roundup(s) together -> unique hotels -> reviews >=2000w -> roundups. Reuses existing phang-nga reviews.',
  phases: [
...
const CLUSTER = "phang-nga", PROV = "พังงา", CRUMB = "พังงา", CRUMBHREF = "city-phang-nga.html";
const POSTS = [{"old":"top10-5stars-khaolak-phangnga-hotels","newSlug":"top10-khao-lak-5star-hotels-phang-nga","title":"10 ที่พักหรู 5 ดาวที่เขาหลัก พังงา","n":10}];
...
const PLAN_SCHEMA = {"type":"object","additionalProperties":false,"required":["hotels","roundups"],"properties":{"hotels":{"type":"array","description":"โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)","items":{"type":"object","additionalProperties":false,"required":["name","reviewSlug","area","tier","star","open"],"properties":{"name":{"type":"string"},"reviewSlug":{"type":"string","description":"review-<hotel-kebab-en>-phang-nga"},"area":{"type":"string"},"tier":{"type":"string"},"star":{"type":"number"},"open":{"type":"boolean"},"note":{"type":"string"}}}},"roundups":{"type":"array","description":"membership ต่อ roundup","items":{"type":"object","additionalProperties":false,"required":["newSlug","reviewSlugs"],"properties":{"newSlug":{"type":"string"},"reviewSlugs":{"type":"array","items":{"type":"string"},"description":"reviewSlug เรียงตามอันดับ"}}}}}};
