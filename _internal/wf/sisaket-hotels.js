export const meta = {
  name: 'sisaket-hotels',
  description: 'Sisaket (ศรีสะเกษ) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Sisaket hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมศรีสะเกษ roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-sisaket.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-sisaket" (EN /en/) · crumbCityName="ศรีสะเกษ"/(EN "Sisaket"), crumbCityHref="city-sisaket.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/sisaket-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
if (existingReviews.length >= 10) {
  ...
  const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:12, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
    name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-sisaket (ต้องไม่ซ้ำกับตัวอื่น)'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

  const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดศรีสะเกษ ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: โรงแรมดีที่สุดในเมือง 2-3 · บูทีค/4 ดาว 3 · กลาง 3-4 · ประหยัด 2 · โฮสเทล 1 — กระจายย่าน (ใจกลางเมืองศรีสะเกษ · ใกล้สถานีรถไฟ · รอบห้วยน้ำคำ · อำเภอกันทรลักษ์)
เลือกที่มีรีวิวจริงและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ (เลือกที่เปิดทั้งปี)
**ต้องได้ 12 โรงแรมที่ slug ไม่ซ้ำกันเลย** — slug = review-<ชื่อโรงแรม-kebab>-sisaket · ถ้าชื่อย่อแล้วเสี่ยงซ้ำ ให้เติมย่าน/จุดเด่นให้ต่างกัน · คืนผลตาม schema (12 รายการพอดี)`,
    { label:'plan:sisaket-hotels', phase:'Plan', schema: PLAN_SCHEMA }
  )
  ...
  const reviewed = await parallel(hotels.map(h => () =>
    agent(
`รีวิวโรงแรม "${h.name}" ศรีสะเกษ · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมศรีสะเกษ" (top10-hotels-sisaket)
...
สร้าง roundup "Top 10 โรงแรมศรีสะเกษ": เขียน astro/src/content/roundups/top10-hotels-sisaket.json (ไทย) + roundups-en/top10-hotels-sisaket.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-sisaket" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-sisaket)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:sisaket', phase:'Roundup' }
)

return { hotels: hotels.length, reviewsOk: okR.length, roundup: 'top10-hotels-sisaket' }
