...
// === Chanthaburi-A (cluster=chanthaburi): 4 beach/coastal roundups ===
const PROV = 'จันทบุรี', CLUSTER = 'chanthaburi', CRUMB = 'จันทบุรี', CRUMBHREF = 'city-chanthaburi.html';
const POSTS = [
  { id:4171, old:'top-4-hotels-kung-wiman-beach-chanthaburi', newSlug:'top4-kung-wiman-beach-hotels-chanthaburi', title:'4 ที่พักริมหาดอ่าวคุ้งวิมาน จันทบุรี', n:4 },
  { id:1006, old:'top10-chao-lao-beach-chanthaburi-thailand', newSlug:'top10-chao-lao-beach-hotels-chanthaburi', title:'10 ที่พักริมหาดเจ้าหลาว จันทบุรี', n:10 },
  { id:1199, old:'top5-khung-chanthaburi-hotels', newSlug:'top5-kung-kraben-community-hotels-chanthaburi', title:'5 ที่พักกินอาหารทะเล วิถีชุมชน อ่าวคุ้งกระเบน จันทบุรี', n:5 },
  { id:1189, old:'top5-chanthaburi-river-hotels', newSlug:'top5-chanthaboon-riverside-hotels-chanthaburi', title:'5 โรงแรมริมน้ำจันทบูร จันทบุรี', n:5 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-chanthaburi'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:chan-a', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/chanthaburi-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
