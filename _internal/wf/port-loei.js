export const meta = {
  name: 'port-loei',
  description: 'Single-planner porter for เลย (cluster=loei): one agent plans all 1 roundup(s) together -> unique hotels -> reviews >=2000w -> roundups. Reuses existing loei reviews.',
  phases: [
...
const POSTS = [{"old":"top10-chiangkhan-loei-hotels","newSlug":"top10-chiang-khan-hotels-loei","title":"10 ที่พักริมโขงบรรยากาศชิลที่เชียงคาน เลย","n":10}];
...
const PLAN_SCHEMA = {"type":"object","additionalProperties":false,"required":["hotels","roundups"],"properties":{"hotels":{"type":"array","description":"โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)","items":{"type":"object","additionalProperties":false,"required":["name","reviewSlug","area","tier","star","open"],"properties":{"name":{"type":"string"},"reviewSlug":{"type":"string","description":"review-<hotel-kebab-en>-loei"},"area":{"type":"string"},"tier":{"type":"string"},"star":{"type":"number"},"open":{"type":"boolean"},"note":{"type":"string"}}}},"roundups":{"type":"array","description":"membership ต่อ roundup","items":{"type":"object","additionalProperties":false,"required":["newSlug","reviewSlugs"],"properties":{"newSlug":{"type":"string"},"reviewSlugs":{"type":"array","items":{"type":"string"},"description":"reviewSlug เรียงตามอันดับ"}}}}}};
