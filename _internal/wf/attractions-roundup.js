...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['restaurants'], properties:{ restaurants:{ type:'array', minItems:10, maxItems:10, items:{
  type:'object', additionalProperties:false, required:['name','slug','area','signature','cuisine','whyPopular'], properties:{
    ...
    signature:{type:'string',description:'ไฮไลต์เด่น สั้น'}, cuisine:{type:'string',description:'ประเภท เช่น ห้างสรรพสินค้า, สวนสาธารณะ, วัด, ตลาด, แลนด์มาร์ก'},
...
// ───────────────────────── Plan ─────────────────────────
phase('Plan')
const plan = await agent(`วิจัยและคัดเลือก **10 ที่เที่ยวยอดนิยมที่สุดใน${PROV}** (คนพูดถึงจริง รีวิวเยอะ เปิดอยู่ปัจจุบัน — ผสมหลากหลาย: ห้าง-ช้อปปิ้ง/สวน-พื้นที่สีเขียว/แลนด์มาร์ก/ตลาด-คอมมูนิตี้มอลล์/จุดถ่ายรูป-ที่เที่ยวกลางคืน)
...
${RULES}
คืนตาม schema (10 ที่เที่ยว เรียงดัง/ไอคอนิกสุดลงไป)`,
...
const written = await parallel(rests.map((r, i) => () =>
  agent(`เขียนรีวิวเชิงลึก + เก็บข้อมูลสถานที่ท่องเที่ยว "${r.name}" (ย่าน ${r.area}, ${PROV}) — ลำดับ ${i+1}/10
...
const frame = await agent(`เขียน "ส่วนกรอบบทความ" (ไม่ใช่เนื้อแต่ละร้าน) สำหรับ **"10 ที่เที่ยวยอดนิยมใน${DISPLAY}"**
ที่เที่ยว 10 แห่ง (อ้างให้ตรงจริง): ${frameInput}
เขียนทุก field ตาม schema เป็นภาษาไทย โทน v2-clean เพื่อนเล่าให้เพื่อนฟัง:
- title "10 ที่เที่ยวยอดนิยมใน${DISPLAY} ${TODAY.slice(0,4)} — ..." ลงท้าย " | ThailandAddict" · h1 ครอบ "${HILITE}" ด้วย <span class="hi"> · eyebrow · intro 2-3 ประโยค
- leadHtml = ย่อหน้านำ "เล่าเรื่อง" 2 ย่อหน้า (<p>…</p><p>…</p>) สไตล์เพื่อนเล่าให้เพื่อน: ย่อหน้าแรกเล่าเสน่ห์ที่เที่ยวของ${PROV} (ย่าน·บรรยากาศ·แนวที่เที่ยวเด่น) ชวนให้อยากมาลอง · ย่อหน้าสองเล่าตำนาน/จุดเด่นของร้านในลิสต์ (ประวัติ·รางวัล·ของขึ้นชื่อที่ยืนยันได้) แล้วชวนมากิน — **ห้ามเขียน disclaimer ลดเครดิตตัวเอง** (ห้ามวลี "ไม่ได้ไปกินเอง / รวบรวมจากรีวิว / ไม่เดาให้ / พูดกว้าง ๆ") และห้ามกุว่าไปกินเอง — บรรยายอย่างมั่นใจจากชื่อเสียงและข้อมูลที่ยืนยันได้
...
- faq 5 ข้อ (ที่ไหนห้ามพลาด/ไฮไลต์${PROV}/ค่าเข้า/การเดินทาง-ที่จอดรถ/เวลาเปิด) ตอบอ้างร้านจริง
- stayTitle (🛏️)+stayText (โมดูลจองที่พัก) · foodTitle+foodText (ตั๋ว/ทัวร์เที่ยวผ่าน Klook/GetYourGuide — เที่ยวหลายจุดมีไกด์/จองตั๋วไม่ต้องต่อคิว)
- **localtips 4-6 ข้อ "รู้ก่อนไปเที่ยวที่${PROV}"** สำหรับต่างชาติ {icon,title,text}: การเดินทาง (Grab/รถ/เดิน), จ่ายเงิน (สตรีท=เงินสด), ช่วงเวลา-เลี่ยงคิว, ทิป, ภาษา/เมนูอังกฤษ
${RULES}
...
if (frame.localtips && frame.localtips.length) blocks.push({ kind:'localtips', title:`รู้ก่อนไปเที่ยวที่${PROV}`, items: frame.localtips })
