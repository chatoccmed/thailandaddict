export const meta = {
  name: 'mae-hong-son-hotels',
  description: 'Mae Hong Son gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Mae Hong Son / Pai hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมแม่ฮ่องสอน roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-mae-hong-son.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-mae-hong-son" (EN ใช้ /en/) · crumbCityName="แม่ฮ่องสอน"/(EN "Mae Hong Son"), crumbCityHref="city-mae-hong-son.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมแม่ฮ่องสอนตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/mae-hong-son-<short>.jpg ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror) อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-mae-hong-son'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดแม่ฮ่องสอนที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
แม่ฮ่องสอนเป็นเมืองเล็กในหุบเขา ที่พักส่วนใหญ่อยู่ที่ "ปาย" (Pai) และ "ตัวเมืองแม่ฮ่องสอน" รอบหนองจองคำ — มีรีสอร์ทวิวภูเขา บูทีค โฮมสเตย์ และโฮสเทลเยอะ
คละระดับให้สมจริงกับพื้นที่: รีสอร์ท/บูทีคหรู 2 · บูทีค/รีสอร์ทวิวภูเขา 4 · กลาง 3 · ประหยัด/โฮมสเตย์ 2 · โฮสเทล 1 — กระจายทำเล (ปายเมือง/ปายชานเมืองวิวเขา/ตัวเมืองแม่ฮ่องสอน/ปางมะผ้า ถ้ามี)
เลือกที่ที่มีรีวิวจริงเยอะและคนไทยรู้จัก/จองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-mae-hong-son
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:mhs-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" แม่ฮ่องสอน · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมแม่ฮ่องสอน" (top10-hotels-mae-hong-son)
...
สร้างหน้า roundup "Top 10 โรงแรมแม่ฮ่องสอน" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-mae-hong-son.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-mae-hong-son.json (อังกฤษ)
...
slug="top10-hotels-mae-hong-son" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-mae-hong-son เพื่อดึงคะแนน/ราคา/ทำเล/ลิงก์จองมาใส่ entries)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:mhs', phase:'Roundup' }
)
