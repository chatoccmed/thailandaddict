export const meta = {
  name: 'phetchaburi-hotels',
  description: 'Phetchaburi gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Phetchaburi / Cha-am hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมเพชรบุรี roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-phetchaburi.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-phetchaburi" (EN ใช้ /en/) · crumbCityName="เพชรบุรี"/(EN "Phetchaburi"), crumbCityHref="city-phetchaburi.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมเพชรบุรีตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/phetchaburi-<short>.jpg ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror) อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-phetchaburi'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดเพชรบุรีที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
เพชรบุรีมีที่พักหลายโซน: "ชะอำ" (รีสอร์ท/โรงแรมริมหาดเยอะที่สุด ตั้งแต่หรูถึงประหยัด) · "ตัวเมืองเพชรบุรี/เมืองเก่า" (บูทีค เกสต์เฮาส์) · "แก่งกระจาน" (รีสอร์ทธรรมชาติริมน้ำ/ในป่า)
คละระดับให้สมจริง: รีสอร์ทหรูริมหาดชะอำ 2 · บูทีค/รีสอร์ท 4 · กลาง 3 · ประหยัด/เกสต์เฮาส์ 2 · โฮสเทล 1 — กระจายทำเล (ชะอำ/ตัวเมือง/แก่งกระจาน)
เลือกที่ที่มีรีวิวจริงเยอะและคนไทยรู้จัก/จองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-phetchaburi
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:pbi-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" เพชรบุรี · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมเพชรบุรี" (top10-hotels-phetchaburi)
...
สร้างหน้า roundup "Top 10 โรงแรมเพชรบุรี" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-phetchaburi.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-phetchaburi.json (อังกฤษ)
...
slug="top10-hotels-phetchaburi" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-phetchaburi เพื่อดึงคะแนน/ราคา/ทำเล/ลิงก์จองมาใส่ entries)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:pbi', phase:'Roundup' }
)
