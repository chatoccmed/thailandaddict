export const meta = {
  name: 'krabi-hotels',
  description: 'Krabi gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup into the content collections',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Krabi hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมกระบี่ roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-krabi.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-krabi" (EN ใช้ /en/) · crumbCityName="กระบี่"/(EN "Krabi"), crumbCityHref="city-krabi.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมกระบี่ตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/krabi-<short>.jpg (ใช้ curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror) อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-krabi'}, area:{type:'string', description:'ย่าน/ทำเล'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในกระบี่ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
คละระดับให้ครบ: หรู 2 · บูทีค/4-5 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 2 — กระจายย่าน (อ่าวนาง/ตัวเมืองกระบี่/ไร่เลย์/เกาะลันตา/คลองม่วง-ทับแขก ฯลฯ)
เลือกที่ที่มีรีวิวจริงเยอะและคนไทย/นักท่องเที่ยวรู้จัก จองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-krabi
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:krabi-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" กระบี่ · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมกระบี่" (top10-hotels-krabi)
...
สร้างหน้า roundup "Top 10 โรงแรมกระบี่" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-krabi.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-krabi.json (อังกฤษ)
...
slug="top10-hotels-krabi" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-krabi เพื่อดึงคะแนน/ราคา/ทำเล/ลิงก์จองมาใส่ entries)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:krabi', phase:'Roundup' }
)
