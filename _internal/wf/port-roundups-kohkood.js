...
// === Koh Kood (cluster=koh-kood): 2 lists ===
const PROV = 'ตราด', CLUSTER = 'koh-kood', CRUMB = 'เกาะกูด', CRUMBHREF = 'city-koh-kood.html';
const POSTS = [
  { id:3061, old:'top15-koh-kud-hotels-tard', newSlug:'top15-hotels-koh-kood', title:'15 ที่พักบรรยากาศดีบนเกาะกูด ตราด', n:15 },
  { id:3064, old:'top5-hotels-koh-kood-trat', newSlug:'top5-budget-hotels-koh-kood', title:'5 ที่พักหลักร้อยบนเกาะกูด ตราด', n:5 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-koh-kood'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:koh-kood', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/koh-kood-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
