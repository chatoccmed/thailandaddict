export const meta = {
  name: 'pai-hotels',
  description: 'Pai (ปาย, แม่ฮ่องสอน) destination — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Pai hotels/resorts across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 ที่พักปาย roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-pai.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-pai" (EN /en/) · crumbCityName="ปาย"/(EN "Pai"), crumbCityHref="city-pai.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: หารูปจริงของโรงแรม (Booking/Agoda/Trip/Google) โหลดไป astro/public/images/hotels/pai-<short>.jpg (curl -m 60 --connect-timeout 20) ครบ hero+gallery ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี gradient placeholder)
- ราคา "เริ่มประมาณ" จากช่วงราคาห้องมาตรฐานจริง ไม่เอาราคาพีคจุดเดียว · ปายราคาห้องสวิงตามฤดู (ไฮซีซั่นหนาวแพง) ใช้ราคาช่วงปกติ
...
if (existingReviews.length >= 10) {
  ...
  const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:12, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
    name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-pai (ต้องไม่ซ้ำกับตัวอื่น และต้องลงท้าย -pai)'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

  const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/รีสอร์ต/เกสต์เฮาส์ในอำเภอปาย (จ.แม่ฮ่องสอน) ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: รีสอร์ตบูทีค/หรูริมน้ำ-ริมนา 3-4 · บูทีคกลางเมือง 3 · กลาง 3 · ประหยัด/บังกะโลริมทุ่ง 1-2 · โฮสเทล/แบ็คแพ็คเกอร์ 1 — กระจายย่าน (กลางเมืองปายใกล้ถนนคนเดิน · ริมแม่น้ำปาย · ทุ่งนาแม่ฮี้/แม่ปิง วิวเขา · ใกล้ปายแคนยอน/สะพานประวัติศาสตร์ · เนินเขาสันติชล)
**ห้ามเลือกซ้ำชื่อโรงแรมเดียวกัน** เลือกที่มีรีวิวจริงและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
**ต้องได้ 12 ที่ slug ไม่ซ้ำกันเลย และเป็นคนละโรงแรมกันจริง ๆ** — slug = review-<ชื่อ-kebab>-pai (ลงท้าย -pai เสมอ ห้ามลงท้าย -mae-hong-son เพราะชนคลัสเตอร์แม่ฮ่องสอน) · คืนผลตาม schema (12 รายการพอดี)`,
    { label:'plan:pai-hotels', phase:'Plan', schema: PLAN_SCHEMA }
  )
  ...
  const reviewed = await parallel(hotels.map(h => () =>
    agent(
`รีวิวโรงแรม "${h.name}" อำเภอปาย แม่ฮ่องสอน · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 ที่พักปาย" (top10-hotels-pai)
...
สร้าง roundup "Top 10 ที่พักปาย": เขียน astro/src/content/roundups/top10-hotels-pai.json (ไทย) + roundups-en/top10-hotels-pai.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-pai" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-pai)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:pai', phase:'Roundup' }
)

return { hotels: hotels.length, reviewsOk: okR.length, roundup: 'top10-hotels-pai' }
