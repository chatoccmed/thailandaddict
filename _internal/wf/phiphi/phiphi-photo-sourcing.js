export const meta = {
  name: 'phiphi-photo-sourcing',
  description: 'Find 4 directly-downloadable real photos per Koh Phi Phi hotel (hero + 3 gallery) from official sites / Trip.com',
  phases: [{ title: 'Sources' }],
}
const RESEARCH = 'C:\\Users\\Imac\\Thailandaddict\\thailandaddict\\_internal\\wf\\phiphi\\research.json'
const SCHEMA = {
  ...
  properties: {
    ...
    found: { type: 'boolean' },
    images: {
      type: 'array', description: 'up to 4 DIRECT image-file URLs (.jpg/.jpeg/.png/.webp) of THIS hotel — ideally hero/exterior/beach, a room, the pool, a view. Real photos actually seen in a fetched page.',
...
const results = await parallel(SLUGS.map(slug => () =>
  agent(
    `Find up to 4 directly-downloadable REAL photos of a specific Koh Phi Phi hotel, for a Thai travel-site review.
