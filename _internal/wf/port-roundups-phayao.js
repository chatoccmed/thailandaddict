...
// === Phayao (cluster=phayao): 1 roundup ===
const PROV = 'พะเยา', CLUSTER = 'phayao', CRUMB = 'พะเยา', CRUMBHREF = 'city-phayao.html';
const POSTS = [
  { id:530, old:'top10-phayao-hotels-thailand', newSlug:'top10-phayao-hotels', title:'10 โรงแรมยอดนิยมในจังหวัดพะเยา', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-phayao'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:phayao', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/phayao-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
