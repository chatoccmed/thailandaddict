export const meta = {
  name: 'samut-songkhram-hotels',
  description: 'Samut Songkhram gold-template — hotel layer: pick 12 real hotels, write TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Samut Songkhram / Amphawa hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมสมุทรสงคราม roundup TH+EN' },
  ],
}
...
- breadcrumb/parent ชี้ roundup: parentHref="top10-hotels-samut-songkhram.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-samut-songkhram" (EN ใช้ /en/) · crumbCityName="สมุทรสงคราม"/(EN "Samut Songkhram"), crumbCityHref="city-samut-songkhram.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมสมุทรสงครามตัวอื่นในชุดนี้ (ใช้ slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/samut-songkhram-<short>.jpg ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror) อย่าใส่ลิงก์ที่ใช้ไม่ได้
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-samut-songkhram'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดสมุทรสงคราม (แม่กลอง) ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับทำชุดรีวิว + จัดอันดับ Top 10 ของ thailandaddict.com
สมุทรสงครามเป็นเมืองเล็กริมน้ำ ที่พักส่วนใหญ่เป็น "โฮมสเตย์/บูทีค/รีสอร์ตริมคลองแถวอัมพวา" และโรงแรมในตัวเมืองแม่กลอง · บางส่วนริมแม่น้ำแม่กลอง/ใกล้ดอนหอยหลอด
คละระดับให้สมจริงกับเมืองเล็ก: บูทีค/รีสอร์ตริมคลองชั้นดี 3 · กลาง 4 · ประหยัด/โฮมสเตย์ริมน้ำ 3 · โฮสเทล/เกสต์เฮาส์ 2 (ถ้ามีหรูจริงค่อยใส่) — กระจายทำเล (อัมพวา/ตัวเมืองแม่กลอง/ริมแม่น้ำ/ดอนหอยหลอด)
เลือกที่ที่มีรีวิวจริงเยอะและคนไทยรู้จัก/จองได้จริง ห้ามเลือกที่ปิดถาวรหรือเปลี่ยนชื่อ
ตั้ง slug = review-<ชื่อโรงแรม-kebab>-samut-songkhram
คืนผลตาม schema (name, slug, area, tier, starTier)`,
  { label:'plan:skm-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" สมุทรสงคราม (แม่กลอง/อัมพวา) · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมสมุทรสงคราม" (top10-hotels-samut-songkhram)
...
สร้างหน้า roundup "Top 10 โรงแรมสมุทรสงคราม" ของ thailandaddict.com
เขียน 2 ไฟล์: astro/src/content/roundups/top10-hotels-samut-songkhram.json (ไทย) และ astro/src/content/roundups-en/top10-hotels-samut-songkhram.json (อังกฤษ)
...
slug="top10-hotels-samut-songkhram" · จัดอันดับจากโรงแรมที่รีวิวไว้แล้วในชุดนี้ (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-samut-songkhram เพื่อดึงคะแนน/ราคา/ทำเล/ลิงก์จองมาใส่ entries)
แต่ละ entry: reviewUrl="<slug ของรีวิว>.html", agodaUrl/bookingUrl/tripUrl ตามที่รีวิวนั้นใช้, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:skm', phase:'Roundup' }
)
