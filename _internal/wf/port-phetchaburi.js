export const meta = {
  name: 'port-phetchaburi',
  description: 'Single-planner porter for เพชรบุรี (cluster=phetchaburi): one agent plans all 1 roundup(s) together -> unique hotels -> reviews >=2000w -> roundups. Reuses existing phetchaburi reviews.',
  phases: [
...
const CLUSTER = "phetchaburi", PROV = "เพชรบุรี", CRUMB = "เพชรบุรี", CRUMBHREF = "city-phetchaburi.html";
const POSTS = [{"old":"top10-hotels-phetchaburi","newSlug":"top10-cha-am-budget-hotels-phetchaburi","title":"10 ที่พักหลักร้อยในชะอำ เพชรบุรี","n":10}];
...
const PLAN_SCHEMA = {"type":"object","additionalProperties":false,"required":["hotels","roundups"],"properties":{"hotels":{"type":"array","description":"โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)","items":{"type":"object","additionalProperties":false,"required":["name","reviewSlug","area","tier","star","open"],"properties":{"name":{"type":"string"},"reviewSlug":{"type":"string","description":"review-<hotel-kebab-en>-phetchaburi"},"area":{"type":"string"},"tier":{"type":"string"},"star":{"type":"number"},"open":{"type":"boolean"},"note":{"type":"string"}}}},"roundups":{"type":"array","description":"membership ต่อ roundup","items":{"type":"object","additionalProperties":false,"required":["newSlug","reviewSlugs"],"properties":{"newSlug":{"type":"string"},"reviewSlugs":{"type":"array","items":{"type":"string"},"description":"reviewSlug เรียงตามอันดับ"}}}}}};
