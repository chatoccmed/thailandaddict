export const meta = {
  name: 'phang-nga-hotels',
  description: 'Phang Nga gold-template — 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    ...
    { title: 'Roundup', detail: 'build Top 10 โรงแรมพังงา roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-phang-nga.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-phang-nga" (EN /en/) · crumbCityName="พังงา"/(EN "Phang Nga"), crumbCityHref="city-phang-nga.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/phang-nga-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-phang-nga'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดพังงาที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: หรู 2-3 · บูทีค/4-5 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 1-2 — กระจายย่าน (เขาหลัก/เกาะยาวน้อย/เกาะยาวใหญ่/ตัวเมืองพังงา/ตะกั่วป่า/ทับละมุ)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
slug = review-<ชื่อโรงแรม-kebab>-phang-nga · คืนผลตาม schema`,
  { label:'plan:phang-nga-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" พังงา · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมพังงา" (top10-hotels-phang-nga)
...
สร้าง roundup "Top 10 โรงแรมพังงา": เขียน astro/src/content/roundups/top10-hotels-phang-nga.json (ไทย) + roundups-en/top10-hotels-phang-nga.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-phang-nga" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-phang-nga)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:phang-nga', phase:'Roundup' }
)
