// Generate a workflow that web-verifies each ranked restaurant/cafe in NON-Bangkok (province) eat-rankings is still open.
// Targets: type=eat-ranking, cluster NOT a Bangkok hood and != 'bangkok', has >=1 restaurant block, slug not top10-attractions-* (temples don't "close").
...
for (const f of fs.readdirSync(d)) {
  ...
  if (a.type !== 'eat-ranking') continue;
  if (hoods.has(a.cluster) || a.cluster === 'bangkok') continue;
  if (/^top\d+-attractions-/.test(a.slug)) continue;
...
lines.push(`  name: 'verify-eat-venues-provinces',`);
lines.push(`  description: 'Web-verify each ranked restaurant still open across ${arts.length} province eat-rankings, ${batches.length} batches. No edits.',`);
...
lines.push(`  agent('ตรวจสอบว่าร้านอาหาร/คาเฟ่ที่จัดอันดับไว้ยังเปิดอยู่จริงไหม (QA) - ตรวจอย่างเดียว ห้ามแก้ไฟล์. บทความ eat-ranking ต่างจังหวัด.\\n' +`);
lines.push(`    'แต่ละร้าน: web-search ชื่อ+จังหวัด/พื้นที่ แล้วประเมิน open = ยังเปิดดำเนินการ (ปิดถาวร/ย้าย/เปลี่ยนชื่อถาวร = false). ปิด/ย้าย/เปลี่ยนชื่อ ให้ใส่ issue สั้นๆ + severity. เปิดปกติ open=true severity=none. อย่าเดา ถ้าไม่ชัวร์ให้ค้นจริง; ร้านเล็ก/สตรีทฟู้ด/ร้านท้องถิ่นที่หาไม่เจอแต่ไม่มีสัญญาณปิด ให้ open=true severity=none.\\n\\n' +`);
...
fs.writeFileSync('_internal/wf/verify-eat-venues-provinces.js', lines.join('\n') + '\n');
console.log('wrote workflow: ' + arts.length + ' articles, ' + batches.length + ' batches, ' + arts.reduce((s, a) => s + a.venues.length, 0) + ' venues');
