export const meta = {
  name: 'ayutthaya-hotels',
  description: 'Ayutthaya gold-template — 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    ...
    { title: 'Roundup', detail: 'build Top 10 โรงแรมอยุธยา roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-ayutthaya.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-ayutthaya" (EN /en/) · crumbCityName="พระนครศรีอยุธยา"/(EN "Ayutthaya"), crumbCityHref="city-ayutthaya.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/ayutthaya-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-ayutthaya'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดพระนครศรีอยุธยาที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: หรู/รีสอร์ตริมน้ำ 2 · บูทีค/4 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 2 — กระจายย่าน (เกาะเมือง/ริมแม่น้ำเจ้าพระยา-ป่าสัก/รอบนอกเมือง/ใกล้สถานีรถไฟ) เน้นบูทีค/เรือนเก่าในเกาะเมือง
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
slug = review-<ชื่อโรงแรม-kebab>-ayutthaya · คืนผลตาม schema`,
  { label:'plan:ayutthaya-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" พระนครศรีอยุธยา · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมอยุธยา" (top10-hotels-ayutthaya)
...
สร้าง roundup "Top 10 โรงแรมอยุธยา": เขียน astro/src/content/roundups/top10-hotels-ayutthaya.json (ไทย) + roundups-en/top10-hotels-ayutthaya.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-ayutthaya" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-ayutthaya)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:ayutthaya', phase:'Roundup' }
)
