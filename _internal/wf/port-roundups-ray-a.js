...
// === Rayong-A (cluster=rayong): city + mainland beaches ===
const PROV = 'ระยอง', CLUSTER = 'rayong', CRUMB = 'ระยอง', CRUMBHREF = 'city-rayong.html';
const POSTS = [
  { id:1160, old:'top10-rayong-hotels', newSlug:'top10-rayong-city-hotels', title:'10 โรงแรมใจกลางเมืองระยอง', n:10 },
  { id:1482, old:'top10-cheap-rayong-hotels', newSlug:'top10-budget-hotels-rayong', title:'10 โรงแรมหลักร้อย ทำเลดีใจกลางเมืองระยอง', n:10 },
  { id:2988, old:'top4-ban-chang-hotels-ra-yong', newSlug:'top4-ban-chang-hotels-rayong', title:'4 ที่พักในอำเภอบ้านฉาง ระยอง', n:4 },
  { id:3412, old:'top5-hotels-rayoung', newSlug:'top5-khao-laem-ya-hotels-rayong', title:'5 ที่พักใกล้อุทยานแห่งชาติเขาแหลมหญ้า ระยอง', n:5 },
  { id:2996, old:'top5-hotels-rayoug', newSlug:'top5-suan-son-beach-hotels-rayong', title:'5 ที่พักริมหาดสวนสน ระยอง', n:5 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-rayong'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:ray-a', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/rayong-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
