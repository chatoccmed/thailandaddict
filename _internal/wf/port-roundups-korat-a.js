...
// === Korat-A (cluster=nakhon-ratchasima): city/phimai/wang-nam-khiao ===
const PROV = 'นครราชสีมา', CLUSTER = 'nakhon-ratchasima', CRUMB = 'นครราชสีมา', CRUMBHREF = 'city-nakhon-ratchasima.html';
const POSTS = [
  { id:700, old:'top10-nakhon-ratchasima-korat-hotels', newSlug:'top10-korat-city-hotels', title:'10 โรงแรมยอดนิยมในอำเภอเมืองนครราชสีมา', n:10 },
  { id:1575, old:'top10-cheap-nakhon-ratchasima-korat-hotels', newSlug:'top10-budget-hotels-korat', title:'10 ที่พักหลักร้อยในเมืองนครราชสีมา', n:10 },
  { id:2194, old:'topfive-hotel-nakhon-ratchasima', newSlug:'top5-phimai-hotels-nakhon-ratchasima', title:'5 ที่พักใกล้ปราสาทหินพิมาย นครราชสีมา', n:5 },
  { id:1105, old:'top10-wang-nam-kheaw-korat-hotels', newSlug:'top10-wang-nam-khiao-hotels-nakhon-ratchasima', title:'10 ที่พักชมหมอกวังน้ำเขียว นครราชสีมา', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-nakhon-ratchasima'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:korat-a', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/korat-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
