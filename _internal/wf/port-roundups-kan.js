...
// === Kanchanaburi (cluster=kanchanaburi): 5 area roundups ===
const PROV = 'กาญจนบุรี', CLUSTER = 'kanchanaburi', CRUMB = 'กาญจนบุรี', CRUMBHREF = 'city-kanchanaburi.html';
const POSTS = [
  { id:4259, old:'top-hotels-thongphaphum-kanchanaburi', newSlug:'top9-thongphaphum-hotels-kanchanaburi', title:'9 ที่พักในทองผาภูมิ กาญจนบุรี', n:9 },
  { id:4127, old:'top-3-kanchanaburi', newSlug:'top3-srinakarin-dam-hotels-kanchanaburi', title:'3 ที่พักริมเขื่อนศรีนครินทร์ กาญจนบุรี', n:3 },
  { id:3951, old:'top10-kanchanaburi-hotels', newSlug:'top10-sai-yok-river-raft-hotels-kanchanaburi', title:'10 ที่พักริมแพ ริมแม่น้ำแคว ไทรโยค กาญจนบุรี', n:10 },
  { id:3895, old:'top5-hostel-kanchanaburi', newSlug:'top4-hostels-kanchanaburi', title:'4 ที่พักสไตล์ Hostel ในเมืองกาญจนบุรี', n:4 },
  { id:1210, old:'top10-sangkhla-buri-kanchanaburi-hotels', newSlug:'top10-sangkhlaburi-hotels-kanchanaburi', title:'10 ที่พักบรรยากาศดีในสังขละบุรี กาญจนบุรี', n:10 },
];
...
const PLAN_SCHEMA={type:'object',additionalProperties:false,required:['hotels','roundups'],properties:{
  hotels:{type:'array',description:'โรงแรม unique ทั้งหมด (1 slug ต่อ 1 โรงแรมจริง ใช้ซ้ำข้าม roundup ได้)',items:{type:'object',additionalProperties:false,required:['name','reviewSlug','area','tier','star','open'],properties:{
    name:{type:'string'},reviewSlug:{type:'string',description:'review-<hotel-kebab>-kanchanaburi'},area:{type:'string'},tier:{type:'string'},star:{type:'number'},open:{type:'boolean'},note:{type:'string'}}}},
...
สำคัญ: โรงแรมเดียวกันต้องได้ reviewSlug **เดียวกันเป๊ะ** ทุกที่ที่อ้างถึง`,
  { label:'plan:kan', phase:'Plan', schema:PLAN_SCHEMA })
...
- รูป hero: curl -m 60 -A "Mozilla/5.0" รูปจริง → astro/public/images/hotels/kanchanaburi-<short>.jpg (hero+gallery 3) แล้ว node _internal/optimize-images.mjs <paths>; ไม่ได้ปล่อย path (onerror)
${STYLE}
