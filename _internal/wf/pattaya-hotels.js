export const meta = {
  name: 'pattaya-hotels',
  description: 'Pattaya (พัทยา, ชลบุรี) destination — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Pattaya hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 ที่พักพัทยา roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-pattaya.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-pattaya" (EN /en/) · crumbCityName="พัทยา"/(EN "Pattaya"), crumbCityHref="city-pattaya.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: หารูปจริงของโรงแรม (Booking/Agoda/Trip/Google) โหลดไป astro/public/images/hotels/pattaya-<short>.jpg (curl -m 60 --connect-timeout 20) ครบ hero+gallery ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี gradient placeholder)
- ราคา "เริ่มประมาณ" จากช่วงราคาห้องมาตรฐานจริง ไม่เอาราคาพีคจุดเดียว · พัทยาราคาห้องสวิงตามฤดู/วันธรรมดา-เสาร์อาทิตย์ ใช้ราคาช่วงปกติ
...
if (existingReviews.length >= 10) {
  ...
  const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:12, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
    name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-pattaya (ต้องไม่ซ้ำกับตัวอื่น และต้องลงท้าย -pattaya)'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

  const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/รีสอร์ตในพัทยา (จ.ชลบุรี) ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: หรู/บีชฟรอนต์ 5 ดาว 3-4 · บูทีค/ดีไซน์ 3 · กลาง 3 · ประหยัด 1-2 · โฮสเทล 1 — กระจายย่าน (หาดพัทยากลางเมืองใกล้ห้าง/วอล์กกิ้งสตรีท · หาดจอมเทียน · นาเกลือ/วงศ์อมาตย์ เงียบติดทะเล · เขาพระตำหนัก · นาจอมเทียน/บางเสร่)
**ห้ามเลือกซ้ำชื่อโรงแรมเดียวกัน** เลือกที่มีรีวิวจริงและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
**ต้องได้ 12 ที่ slug ไม่ซ้ำกันเลย และเป็นคนละโรงแรมกันจริง ๆ** — slug = review-<ชื่อ-kebab>-pattaya (ลงท้าย -pattaya เสมอ ห้ามลงท้าย -chonburi เพราะชนคลัสเตอร์ชลบุรี) · คืนผลตาม schema (12 รายการพอดี)`,
    { label:'plan:pattaya-hotels', phase:'Plan', schema: PLAN_SCHEMA }
  )
  ...
  const reviewed = await parallel(hotels.map(h => () =>
    agent(
`รีวิวโรงแรม "${h.name}" พัทยา ชลบุรี · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 ที่พักพัทยา" (top10-hotels-pattaya)
...
สร้าง roundup "Top 10 ที่พักพัทยา": เขียน astro/src/content/roundups/top10-hotels-pattaya.json (ไทย) + roundups-en/top10-hotels-pattaya.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-pattaya" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-pattaya)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:pattaya', phase:'Roundup' }
)

return { hotels: hotels.length, reviewsOk: okR.length, roundup: 'top10-hotels-pattaya' }
