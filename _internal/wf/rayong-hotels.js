export const meta = {
  name: 'rayong-hotels',
  description: 'Rayong gold-template — 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    ...
    { title: 'Roundup', detail: 'build Top 10 โรงแรมระยอง roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-rayong.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-rayong" (EN /en/) · crumbCityName="ระยอง"/(EN "Rayong"), crumbCityHref="city-rayong.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/rayong-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-rayong'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดระยองที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: หรู/รีสอร์ตริมทะเล 2-3 · บูทีค/4 ดาว 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 1-2 — กระจายย่าน (เกาะเสม็ด หาดทรายแก้ว/อ่าววงเดือน/หาดแม่รำพึง/บ้านเพ/ตัวเมืองระยอง)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
slug = review-<ชื่อโรงแรม-kebab>-rayong · คืนผลตาม schema`,
  { label:'plan:rayong-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" ระยอง · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมระยอง" (top10-hotels-rayong)
...
สร้าง roundup "Top 10 โรงแรมระยอง": เขียน astro/src/content/roundups/top10-hotels-rayong.json (ไทย) + roundups-en/top10-hotels-rayong.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-rayong" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-rayong)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:rayong', phase:'Roundup' }
)
