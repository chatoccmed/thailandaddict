export const meta = {
  name: 'nong-bua-lamphu-hotels',
  description: 'Nong Bua Lamphu (หนองบัวลำภู) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Nong Bua Lamphu hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมหนองบัวลำภู roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-nong-bua-lamphu.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-nong-bua-lamphu" (EN /en/) · crumbCityName="หนองบัวลำภู"/(EN "Nong Bua Lamphu"), crumbCityHref="city-nong-bua-lamphu.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/nong-bua-lamphu-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
if (existingReviews.length >= 10) {
  ...
  const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:12, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
    name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-nong-bua-lamphu (ต้องไม่ซ้ำกับตัวอื่น)'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

  const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดหนองบัวลำภู ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับเท่าที่เมืองเล็กมีจริง: โรงแรมดีที่สุดในเมือง 2-3 · กลาง 4-5 · ประหยัด 3-4 · รีสอร์ต/โฮสเทล 1-2 — กระจายย่าน (ใจกลางเมืองหนองบัวลำภู รอบหนองบัว · ใกล้ศาลสมเด็จพระนเรศวร · เส้นทางภูพานคำ-อ่างเก็บน้ำอุบลรัตน์ · อำเภอรอบนอก)
...
**ต้องได้ 12 ที่ slug ไม่ซ้ำกันเลย** — slug = review-<ชื่อ-kebab>-nong-bua-lamphu · ถ้าชื่อย่อแล้วเสี่ยงซ้ำ ให้เติมย่าน/จุดเด่นให้ต่างกัน · คืนผลตาม schema (12 รายการพอดี)`,
    { label:'plan:nong-bua-lamphu-hotels', phase:'Plan', schema: PLAN_SCHEMA }
  )
  ...
  const reviewed = await parallel(hotels.map(h => () =>
    agent(
`รีวิวโรงแรม "${h.name}" หนองบัวลำภู · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมหนองบัวลำภู" (top10-hotels-nong-bua-lamphu)
...
สร้าง roundup "Top 10 โรงแรมหนองบัวลำภู": เขียน astro/src/content/roundups/top10-hotels-nong-bua-lamphu.json (ไทย) + roundups-en/top10-hotels-nong-bua-lamphu.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-nong-bua-lamphu" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-nong-bua-lamphu)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:nong-bua-lamphu', phase:'Roundup' }
)

return { hotels: hotels.length, reviewsOk: okR.length, roundup: 'top10-hotels-nong-bua-lamphu' }
