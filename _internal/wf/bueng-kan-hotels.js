export const meta = {
  name: 'bueng-kan-hotels',
  description: 'Bueng Kan (บึงกาฬ) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Bueng Kan hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมบึงกาฬ roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-bueng-kan.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-bueng-kan" (EN /en/) · crumbCityName="บึงกาฬ"/(EN "Bueng Kan"), crumbCityHref="city-bueng-kan.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/bueng-kan-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:12, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-bueng-kan (ต้องไม่ซ้ำกับตัวอื่น)'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดบึงกาฬ ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
บึงกาฬเป็นเมืองเล็ก ที่พักส่วนใหญ่เป็นโรงแรมกลาง/ประหยัด/รีสอร์ตริมโขง/บูทีค — คละ: ดีที่สุดในเมือง/ริมโขง 2-3 · บูทีค/รีสอร์ต 3 · กลาง 4 · ประหยัด/เกสต์เฮาส์ 2-3 (ถ้าหา 5 ดาวไม่ได้ไม่เป็นไร เลือกที่รีวิวดีจริง) — กระจายย่าน (ในเมืองบึงกาฬ · ริมแม่น้ำโขง · ใกล้ภูทอก/เซกา/บึงโขงหลง)
เลือกที่มีรีวิวจริงและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ
**ต้องได้ 12 โรงแรมที่ slug ไม่ซ้ำกันเลย** — slug = review-<ชื่อโรงแรม-kebab>-bueng-kan · ถ้าชื่อย่อแล้วเสี่ยงซ้ำ ให้เติมย่าน/จุดเด่นให้ต่างกัน · คืนผลตาม schema (12 รายการพอดี)`,
  { label:'plan:bueng-kan-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" บึงกาฬ · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมบึงกาฬ" (top10-hotels-bueng-kan)
...
สร้าง roundup "Top 10 โรงแรมบึงกาฬ": เขียน astro/src/content/roundups/top10-hotels-bueng-kan.json (ไทย) + roundups-en/top10-hotels-bueng-kan.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-bueng-kan" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-bueng-kan)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:bueng-kan', phase:'Roundup' }
)
