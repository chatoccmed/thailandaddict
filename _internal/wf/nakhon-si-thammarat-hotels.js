export const meta = {
  name: 'nakhon-si-thammarat-hotels',
  description: 'Nakhon Si Thammarat (เมืองคอน) gold-template — hotel layer: 12 real hotels TH+EN reviews + Top 10 roundup',
  phases: [
    { title: 'Plan',    detail: 'pick 12 currently-operating Nakhon Si Thammarat/Khanom/Sichon hotels across tiers' },
    { title: 'Review',  detail: 'one hotel-reviewer agent per hotel → TH+EN review JSON' },
    { title: 'Roundup', detail: 'build Top 10 โรงแรมนครศรีธรรมราช roundup TH+EN' },
  ],
}
...
- breadcrumb/parent: parentHref="top10-hotels-nakhon-si-thammarat.html", parentCrumbUrl ไทย "https://thailandaddict.com/top10-hotels-nakhon-si-thammarat" (EN /en/) · crumbCityName="นครศรีธรรมราช"/(EN "Nakhon Si Thammarat"), crumbCityHref="city-nakhon-si-thammarat.html" · countryHref="country-thailand.html", countryLabel="🇹🇭 ไทย"(EN "🇹🇭 Thailand"), countryGuideLabel="คู่มือเที่ยวไทย"(EN "Thailand Guide") · addressCountry="TH"
- related/prev/next ลิงก์ไปรีวิวโรงแรมในชุดนี้ (slug ที่ planner กำหนด)
- รูป hero: พยายามโหลดจริงไป astro/public/images/hotels/nakhon-si-thammarat-<short>.jpg (curl -m 60 --connect-timeout 20) ถ้าโหลดไม่ได้ปล่อยว่าง (layout มี onerror)
...
const PLAN_SCHEMA = { type:'object', additionalProperties:false, required:['hotels'], properties:{ hotels:{ type:'array', minItems:10, maxItems:12, items:{ type:'object', additionalProperties:false, required:['name','slug','area','tier','starTier'], properties:{
  name:{type:'string'}, slug:{type:'string', description:'review-<hotel-kebab>-nakhon-si-thammarat'}, area:{type:'string'}, tier:{type:'string', enum:['luxury','boutique','midrange','budget','hostel']}, starTier:{type:'number'} } } } } }

const plan = await agent(
`ค้นเว็บ (Booking/Agoda/Trip.com) เลือกโรงแรม/ที่พักในจังหวัดนครศรีธรรมราช ที่ "เปิดดำเนินการอยู่จริงตอนนี้" 12 แห่ง สำหรับชุดรีวิว + Top 10 ของ thailandaddict.com
คละระดับ: รีสอร์ตทะเลขนอม 2-3 · บูทีค/4 ดาวในเมืองคอน 3 · กลาง 3 · ประหยัด 2 · โฮสเทล 1 — กระจายย่าน (ในเมืองนครศรีธรรมราช/เมืองคอน · ขนอม ริมทะเล · สิชล ใกล้ไอ้ไข่ · คีรีวง/พรหมคีรี)
เลือกที่มีรีวิวจริงเยอะและจองได้จริง ห้ามที่ปิดถาวร/เปลี่ยนชื่อ (ขนอมมีรีสอร์ตริมทะเลเยอะ — เลือกที่เปิดทั้งปีและจองออนไลน์ได้)
slug = review-<ชื่อโรงแรม-kebab>-nakhon-si-thammarat · คืนผลตาม schema`,
  { label:'plan:nakhon-si-thammarat-hotels', phase:'Plan', schema: PLAN_SCHEMA }
)
...
const reviewed = await parallel(hotels.map(h => () =>
  agent(
`รีวิวโรงแรม "${h.name}" นครศรีธรรมราช · slug=${h.slug} · ระดับ ${h.starTier} ดาว (${h.tier}) · ย่าน ${h.area}
อยู่ในชุด roundup "Top 10 โรงแรมนครศรีธรรมราช" (top10-hotels-nakhon-si-thammarat)
...
สร้าง roundup "Top 10 โรงแรมนครศรีธรรมราช": เขียน astro/src/content/roundups/top10-hotels-nakhon-si-thammarat.json (ไทย) + roundups-en/top10-hotels-nakhon-si-thammarat.json (อังกฤษ)
ดู schema (roundupSchema) + รูปแบบ _internal/templates/roundup.sample.json — ตรงทุก field
slug="top10-hotels-nakhon-si-thammarat" · จัดอันดับจากรีวิวในชุด (อ่าน JSON ใน astro/src/content/reviews/ ที่ slug ขึ้นต้น review-...-nakhon-si-thammarat)
แต่ละ entry: reviewUrl="<slug>.html", agodaUrl/bookingUrl/tripUrl ตามรีวิว, score/stars/price จริง
${OVERRIDES}
โรงแรมในชุด:\n${slugList}`,
  { label:'roundup:nakhon-si-thammarat', phase:'Roundup' }
)
