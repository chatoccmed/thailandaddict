...
## 🟢 ACTIVE
- **[2026-08-27 · Claude session · thailandaddict repo] 🟡 IN PROGRESS — "ลงตัว" cliché rewrite across TH hotel reviews.** 294 review files / 448 occurrences of the word being used as a generic filler ending ("...ลงตัว") rather than a specific claim. Rewriting each via 15 parallel agents (batches in scratchpad, ~20 files/agent) — real per-hotel specifics grounded in that same file's own content, no fabrication, no full-file JSON reformatting (string-replacement only). **If you're another session/machine: do not touch `astro/src/content/reviews/*.json` until this is marked DONE** — will commit+push in one batch when finished, or leave partial-progress notes here if interrupted.
