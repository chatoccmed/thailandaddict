// Generate astro/public/sitemap.xml covering all TH + EN pages with hreflang alternates.
// Clean URLs (no .html) to match the canonical convention. Run: node _internal/gen-sitemap.mjs
...
// 1) content slugs (TH set + which have EN)
const sets = [
  ['articles', 'articles-en'],
  ['reviews', 'reviews-en'],
  ['roundups', 'roundups-en'],
];
const pairs = [];                       // {slug, en:boolean}
for (const [th, en] of sets) {
  const enSet = new Set(C(path.join(ROOT, 'astro/src/content', en)));
  for (const slug of C(path.join(ROOT, 'astro/src/content', th))) pairs.push({ slug, en: enSet.has(slug) });
}
...
for (const name of rootHtml) pairs.push({ slug: name, en: enHtml.has(name) });

// 3) homepage (index → '')
pairs.unshift({ slug: '', en: enHtml.has('index'), home: true });

const esc = s => s.replace(/&/g, '&amp;');
function urlEntry(loc, hasEn, thPath, enPath) {
  const alts = hasEn
...
for (const p of pairs) {
  ...
  entries.push(urlEntry(thPath, p.en, thPath, enPath));     // TH url
  if (p.en) entries.push(urlEntry(enPath, true, thPath, enPath)); // EN url
}
...
console.log(`sitemap.xml written · ${entries.length} <url> entries (TH ${thCount} + EN ${enCount}) · ${(xml.length/1024/1024).toFixed(2)} MB`);
