...
- ✅ **#12 hub entity-linking — DONE & LIVE (deploy `a52a3cf1`):** Wikidata + en/th Wikipedia `sameAs` on all 77 province hubs (TH+EN), verified QIDs (`province-wikidata.json` via `fetch-wikidata.mjs`); 4 stale gold resto twins refreshed. (`geo` was already done; `aggregateRating` on a Place = schema-risky, skip; `containsPlace` per province still doable.)
- **Doable next (lower priority):** `containsPlace` per province hub (list each province's notable attractions in Place schema) · Wikidata `sameAs` for the 12 tourist DESTINATIONS hubs (needs per-entity-type lookup) · broader de-orphan (national-guides panel into country-thailand + map bangkok micro-areas) · #16 deep-link Klook/GYG (partly owner-gated) · more niche guides (cooking classes, diving)
