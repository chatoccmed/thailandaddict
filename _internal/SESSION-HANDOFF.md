# SESSION HANDOFF — start here (last updated 2026-06-26, ~13:05, after the Wikidata-hub deploy)

> New session: read this first, then `_internal/DEVELOPMENT-PLAN-v2.md` (full roadmap) + the memory files. Site is LIVE, repo is CLEAN (everything committed + pushed to origin/main), latest deploy version `a52a3cf1`. Nothing is half-done.
...
- **#12 hub entity-linking: Wikidata + Wikipedia `sameAs` on all 77 province hubs** (deploy `a52a3cf1`, TH+EN): added verified `schema.org Place.sameAs` (Wikidata QID + en/th Wikipedia) to each city hub's JSON-LD — entity-disambiguation for Google's knowledge graph + AI engines. QIDs in `_internal/province-wikidata.json`, fetched via `_internal/fetch-wikidata.mjs` (scores candidate descriptions to pick the *province* entity, not the city/island; 77/77, 0 dupes, all verified "province in/of Thailand"; Bangkok = capital). Nothing fabricated. ALSO refreshed the 4 stale gold resto twins to current schema → bilingual eat-ranking cluster 100% consistent. (Remaining #12 ideas: `containsPlace` per province is safe+doable; `aggregateRating` on a Place is schema-risky — skip. The 12 tourist DESTINATIONS hubs (samui/pattaya/…) could get sameAs too, but need per-entity-type lookup — not in the province sidecar.)
