...
//      and the wrapped-link count must be ≥ FLOOR (proves the layout wrappers actually ran)
// Run before every deploy: node _internal/qa/check-booking-cj.mjs [distDir]
//
// A "wrapped" link is either form — both attribute to CJ PID 101809619:
//   /go/b?u=<encoded booking url>&sid=<slug>   ← CURRENT. worker.js 302s to CJ; the ad id lives ONLY in the
//                                                worker, so a CJ change is a 10-line edit, not a 17k-page rebuild.
//   anrdoezrs.net/click-101809619-<adid>?sid=  ← LEGACY inline form; still counted so a partially-migrated
//                                                tree doesn't trip the floor while pages are being regenerated.
import fs from 'node:fs';
import path from 'node:path';
const FLOOR = 4000;
// Both wrapper forms count toward the floor. /go/b is matched on the route + the encoded booking.com payload,
// so a malformed /go/b (missing or non-booking u=) is NOT credited as a working wrapper.
const WRAPPED_RE = /(?:href|data-href)="\/go\/b\?u=https%3A%2F%2Fwww\.booking\.com|anrdoezrs\.net\/click-101809619-\d+\?sid=/g;
let fail = 0;
...
// 2. static hubs wrapped
{
  ...
  for (const dir of ['astro/public', 'astro/public/en']) {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      raw += r;
      wrapped += (s.match(WRAPPED_RE) || []).length;
    }
  }
  if (raw) bad(`static hubs: ${raw} RAW booking.com hrefs (run _internal/qa/wrap-booking-cj.mjs — or gen-hubs regenerated without the goB wrapper)`);
...
if (dist && fs.existsSync(dist)) {
  ...
  const walk = (d) => {
    for (const f of fs.readdirSync(d)) {
      ...
      raw += r;
      cj += (s.match(WRAPPED_RE) || []).length;
    }
  };
  walk(dist);
  if (raw) bad(`dist: ${raw} RAW booking.com CTAs escaped the CJ wrappers (layouts/hubs regression)`);
  else if (cj < FLOOR) bad(`dist: only ${cj} wrapped links (< floor ${FLOOR}) — wrappers likely not running`);
  else console.log(`  ✓ dist wrapped (${cj} wrapped links across ${scanned} pages, 0 raw)`);
} else if (dist) {
