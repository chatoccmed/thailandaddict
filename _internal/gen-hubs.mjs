...
const goB = (u, sid) => u && /booking\.com/.test(u)
  ...
  : u;
/* The booking cards sit on a page about ONE place, yet opened each OTA's
   homepage on every city hub - 16,865 built pages carried that link. Nothing
   was broken, which is why it lasted: the links worked and kept their
   affiliate tags. They simply discarded the one thing the page knows. The
   Bangkok hood block already did this correctly for Agoda and Booking (see
   agoda() further down), so this is that same treatment applied to the city
   hubs, and to Trip.com which was still a homepage in both blocks.
   A search is used rather than Agoda's /city/<slug> page because the slug
...
function provinceHub(slug, th, r, d){
  ...
  const _legend=[['s',tx('ที่พัก','Stays'),_byKind.s||0],['e',tx('ที่กิน','Food'),_byKind.e||0],['a',tx('ที่เที่ยว','Places'),_byKind.a||0]]
    .filter(x=>x[2]>0).map(x=>'<span class="cmap-key cmap-k-'+x[0]+'">'+x[1]+' <b>'+x[2]+'</b></span>').join('');
...
function hoodHub(hood){
  ...
  // real articles clustered under this ย่าน slug (top10-attractions/cafes/restaurants + any itinerary/attraction pieces) —
  // these exist in ARTS[hood] already via each article's own `cluster` field but were previously never surfaced here.
  const hArts=ARTS[hood]||[];
  const hAttrCount=hArts.filter(a=>a.type==='attraction').length, hEatCount=hArts.filter(a=>['food','eat-ranking'].includes(a.type)).length, hPlanCount=hArts.filter(a=>a.type==='itinerary').length;
...
<div class="tabwrap"><div class="tabbar">${tab('stay','🏨',tx('ที่พัก','Stays'),hotels.length,true)}${tab('see','📍',tx('ที่เที่ยว','See'),highlights.length+hAttrCount,false)}${tab('eat','🍜',tx('ที่กิน','Eat'),foods.length+hEatCount,false)}${tab('plan','🗺️',tx('แผนเที่ยว','Plan'),hPlanCount,false)}${tab('prep','🎒',tx('เดินทาง','Around'),0,false)}</div></div>
<div class="cwrap">
...
<section class="panel" id="p-see"><h2 class="pnhead">${tx(`ที่เที่ยวย่าน<em>${nm}</em>`,`Things to do in <em>${nm}</em>`)}</h2><p class="pintro">${tx('ไฮไลต์ แลนด์มาร์ก และจุดเด่นที่นักท่องเที่ยวนิยมในย่านนี้','Highlights and spots visitors love in this area')}</p>${hgHl?`<div class="hoodgrid">${hgHl}</div>`:''}${artCards(hood,['attraction'])||(hgHl?'':`<p class="pintro">${tx('ที่เที่ยวย่านนี้กำลังเพิ่ม','Sights coming soon')}</p>`)}</section>
<section class="panel" id="p-eat"><h2 class="pnhead">${tx(`ที่กินย่าน<em>${nm}</em>`,`Where to eat in <em>${nm}</em>`)}</h2><p class="pintro">${tx('ร้านเด็ดและของกินที่ย่านนี้ขึ้นชื่อ','The dishes and spots this area is known for')}</p>${fgFood?`<div class="foodgrid">${fgFood}</div>`:''}${artCards(hood,['food','eat-ranking'])||(fgFood?'':`<p class="pintro">${tx('ที่กินย่านนี้กำลังเพิ่ม','Food picks coming soon')}</p>`)}</section>
<section class="panel" id="p-plan"><h2 class="pnhead">${tx(`แผนเที่ยวย่าน<em>${nm}</em>`,`<em>${nm}</em> itineraries`)}</h2><p class="pintro">${tx('แผนเดินเที่ยวย่านนี้แบบครึ่งวัน-เต็มวัน','Half-day and full-day routes for this area')}</p>${artCards(hood,['itinerary'])||`<p class="pintro">${tx('แผนเที่ยวย่านนี้กำลังจัดทำ — รีวิวจะตามมา','Itineraries for this area are in the works — coming soon')}</p><div class="callout"><div><h3>${tx('ระหว่างนี้','Meanwhile')}</h3><p>${tx('ดูแผนเที่ยวกรุงเทพ','See Bangkok itineraries')}</p></div><a href="city-bangkok.html">${tx('แผนเที่ยวกรุงเทพ →','Bangkok plans →')}</a></div>`}</section>
